...
export function ProcessCardFace({ step, title, body, hint }: ProcessCardFaceProps) {
  return (
    <div className="process-card-face flex h-full flex-col p-7 text-left md:p-9">
      ...
      {title ? (
        <h3 className="mt-auto font-serif-display text-2xl font-bold text-foreground sm:text-3xl">
          {title}
        </h3>
      ) : (
        <p className="mt-5 text-sm leading-relaxed text-foreground/75">{body}</p>
      )}

      {hint ? (
        <span className="mt-4 inline-flex items-center gap-2 font-mono text-[10px] uppercase tracking-[0.2em] text-foreground/60">
